...
- Devising a procedure, which generates iterative process, it is useful to approach problem from the "bottom-up". For example, in factorial procedure above, when computing with recursion, we thought in terms of `n! = n * (n-1)..`, meaning starting from the top part or the answer and going down. However, for iteration, we start from 1 and go up until the answer, i.e. in terms of `n! = 1 * 2 ... * n`.
