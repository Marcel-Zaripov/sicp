...
; used by the program, so that we could compute the number of ways
; to change a British pound, for example. As the program is written,
; the knowledge of the currency is distributed partly into
; the procedure first-denomination and partly into
; the procedure count-change
; (which knows that there are five kinds of U.S. coins).
; It would be nicer to be able to supply a list
...
; It will still have the same form, but it will access
...
; helpers for the function (THE TASK ITSELF):
(define no-more? null?)
(define except-first-denomination cdr)
(define first-denomination car)

; combiner that takes coins list and returns procedure that can be applied to amount
...
(define us-change (count-change us-coins))
